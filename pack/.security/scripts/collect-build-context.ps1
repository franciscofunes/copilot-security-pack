...
$changedFiles = @((git -C $root status --porcelain=v1 --untracked-files=all 2>$null) | ForEach-Object { if ($_.Length -gt 3) { $_.Substring(3) } } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
...
$github = [ordered]@{ status='unavailable'; correlation='none'; runs=@(); pr=$null; checks=@(); errors=@() }
$azure = [ordered]@{ status='unavailable'; correlation='none'; runs=@(); errors=@() }
$jfrog = [ordered]@{ status='unavailable'; builds=@(); errors=@() }
...
if (Test-CommandAvailable $GitHubCommand) {
    ...
    if ($auth.ok) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($headSha)) {
            ...
            if ($exact.ok) { $github.runs=@($exact.data); if ($github.runs.Count -gt 0) { $github.correlation='exact-head' } }
        ...
        if ($github.runs.Count -eq 0 -and -not [string]::IsNullOrWhiteSpace($branch)) {
            ...
            if ($byBranch.ok) { $github.runs=@($byBranch.data); if ($github.runs.Count -gt 0) { $github.correlation='branch' } }
...
if (Test-CommandAvailable $AzureCommand) {
    ...
    elseif ($detached) { $azure.status='no-branch' }
        ...
        if ($null -eq $azureConfig -and -not $originLooksAzure) { $azure.status='not-configured' }
        else {
...
if (Test-CommandAvailable $JFrogCommand) {
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$buildName)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($buildNumber)) {
            $scanArgs=@('build-scan',[string]$buildName,$buildNumber,'--format=json','--vuln')
            ...
            $jfrog.builds=@([pscustomobject]@{ name=[string]$buildName; number=$buildNumber; source=$source; scanStatus=$(if (-not $scan.ok) {'failed'} elseif ($scan.exitCode -eq 3) {'policy-violation'} else {'completed'}); exitCode=$scan.exitCode; evidenceFile=$evidenceFile })
        }
    }
}
