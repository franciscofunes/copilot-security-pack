...
# GitHub CLI is read-only here. All commands execute from the target repository so
# gh resolves the correct repository even when the VS Code terminal cwd is elsewhere.
    ...
    if ($auth.ok) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($branch)) {
            ...
            if ($pr.ok -and $null -ne $pr.data) {
                $github.pr = $pr.data
                # gh pr checks exits 8 while checks are pending. The JSON is still valid evidence.
...
# Azure DevOps authentication is intentionally not inferred from `az account show`.
# The Azure DevOps extension can authenticate with az login, az devops login, or
# AZURE_DEVOPS_EXT_PAT. We verify that the extension is already installed, then run
# the read-only pipeline query and classify its outcome without interactive login.
...
# JFrog Build-Info identity is repository-specific and is never guessed. The config
# must supply a build name and either a fixed build number or an explicit mapping from
# correlated GitHub/Azure build metadata.
if (Test-CommandAvailable $JFrogCommand) {
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$buildName)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($buildNumber)) {
            ...
            # JFrog build-scan exit 3 means Xray policy violations matched. That is a
            # completed scan with failing security evidence, not an execution failure.
